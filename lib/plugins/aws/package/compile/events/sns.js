            ...
            deliveryPolicy: {
              ...
              properties: {
                healthyRetry: {
                  type: 'object',
                  properties: {
                    minDelayTarget: { type: 'integer', minimum: 1, default: 20 },
                    maxDelayTarget: { type: 'integer', maximum: 3600, default: 20 },
                    numRetries: { type: 'integer', minimum: 0, maximum: 100, default: 3 },
                    numNoDelayRetries: { type: 'integer', minimum: 0, default: 0 },
                    numMinDelayRetries: { type: 'integer', minimum: 0, default: 0 },
                    numMaxDelayRetries: { type: 'integer', minimum: 0, default: 0 },
                    backoffFunction: {
                      type: 'string',
                ...
                throttle: {
                  type: 'object',
                  properties: {
                    maxReceivesPerSecond: { type: 'integer', minimum: 1 },
                  },
                  additionalProperties: false,
                },
                additionalProperties: false,
              },
              anyOf: [{ required: ['healthyRetry'] }, { required: ['throttle'] }],
              additionalProperties: false,
            },
        ...
        functionObj.events.forEach((event) => {
          if (event.sns) {
            ...
            if (topicArn) {
              _.merge(template.Resources, {
                [subscriptionLogicalId]: {
                  ...
                  Properties: {
                    ...
                    Protocol: 'lambda',
                    Endpoint: endpoint,
                    DeliveryPolicy: event.sns.deliveryPolicy,
              ...
              if (event.sns.filterPolicy || redrivePolicy || event.sns.deliveryPolicy) {
                _.merge(template.Resources, {
                  [subscriptionLogicalId]: {
                    Type: 'AWS::SNS::Subscription',
